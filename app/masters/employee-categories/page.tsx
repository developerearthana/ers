...
export default function EmployeeCategoriesMaster() {
    ...
    const loadCategories = async () => {
        setLoading(true);
        const res = await getMasters('EmployeeCategory');
    ...
    return (
        <div className="space-y-6 p-2">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-foreground">Employee Categories</h1>
                    <p className="text-muted-foreground mt-1">Define employment types used when adding employees.</p>
                </div>
                <button
                                    ...
                                    <tr key={cat._id} className="group hover:bg-muted/20 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-muted-foreground">
                                            {editingId === cat._id ? (
                                                ...
                                                cat.metadata?.description || <span className="text-muted-foreground/40">—</span>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-1">
                                                {editingId === cat._id ? (
                                                    <>
                                                        <button onClick={() => handleEditSave(cat._id)} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Save">
                                                    ...
                                                    <>
                                                        <button
                                                            onClick={() => { setEditingId(cat._id); setEditName(cat.label); setEditDesc(cat.metadata?.description || ''); }}
                                ...
                                {categories.length === 0 && (
                                    <tr>
                                        <td colSpan={3} className="p-10 text-center text-muted-foreground">
                                            No employee categories yet. Click "Add Category" to create one.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
